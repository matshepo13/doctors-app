...
declare module 'expo-router' {
  export namespace ExpoRouter {
    export interface __routes<T extends string = string> extends Record<string, unknown> {
      StaticRoutes: `/` | `/(authenticated)` | `/(authenticated)/consultations` | `/(authenticated)/id-entry` | `/(authenticated)/resultsdisplay` | `/(authenticated)/userService` | `/(login)` | `/(login)/login` | `/(login)/loginstyle` | `/(login)/signup` | `/(tabs)` | `/(tabs)/two` | `/_sitemap` | `/consultations` | `/id-entry` | `/login` | `/loginstyle` | `/modal` | `/pages/appointments` | `/resultsdisplay` | `/signup` | `/two` | `/userService`;
